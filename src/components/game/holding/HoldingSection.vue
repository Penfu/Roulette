...
import AmountButton from '@/components/game/holding/AmountButton.vue';
...
const { holding } = storeToRefs(game);
const { allInHolding, resetHolding } = game;

const holdingToDisplay = ref(0);
watch(
  () => holding.value,
  (newHolding, previousHolding) => {
    anime({
      targets: holdingToDisplay,
      value: [previousHolding, newHolding],
      round: 1,
      duration: 500,
      easing: 'linear',
    });
  ...
  <div class="space-y-2">
    ...
    <section class="px-4 py-2 flex flex-col xs:flex-row xs:justify-between gap-8 md:gap-16 text-xl">
      <div class="w-full flex flex-wrap gap-2">
        <AmountButton v-for="amount in amounts" :key="amount" :value="amount" :onSelect="handleSelectAmount" />
      </div>
      ...
      <div class="w-auto flex flex-col lg:flex-row lg:justify-end gap-2">
        ...
        <div class="flex space-x-2">
          ...
          <button
            @click="handleResetHolding"
            aria-label="Reset holding"
