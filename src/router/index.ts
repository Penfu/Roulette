...
const authGuard = (to: any, from: any, next: any) => (useAuthStore().isAuth ? next() : next({ name: 'login' }));
const guestGuard = (to: any, from: any, next: any) => (!useAuthStore().isAuth ? next() : next({ name: 'home' }));
