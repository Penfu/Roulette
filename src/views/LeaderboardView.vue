...
export default {
  ...
  methods: {
    async setUsers() {
      ...
      this.users = response.data.sort((a: { balance: number; }, b: { balance: number; }) => b.balance - a.balance);
    },
  },
};
</script>
...
<template>
  <main class="mx-4 md:mx-8 lg:mx-16 xl:mx-32">
    ...
    <div class="flex flex-col lg:flex-row">
      <div v-for="user in podium" :key="user.name"
      class="w-full py-10 flex items-center justify-center bg-gray-100"
      :class="{ 'bg-yellow-400': podium.indexOf(user) == 0}"
      >
        <div class="basis-1/2 flex items-center space-x-16">
          <span class="w-full basis-1/3 text-5xl font-bold">{{ podium.indexOf(user) + 1 }}</span>
    ...
    <div class="mt-10 mx-4 flex flex-col space-y-2 items-center">
      <div v-for="user in others" :key="user.name" class="w-full flex space-x-8">
